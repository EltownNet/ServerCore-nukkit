...
public class BanCommand extends PluginCommand<ServerCore> {

    public BanCommand(final ServerCore owner) {
        super("ban", owner);
    ...
    public boolean execute(CommandSender sender, String commandLabel, String[] args) {
        ...
        if (sender instanceof Player) {
            ...
            final CustomForm formWindowCustom = new CustomForm.Builder("§7» §8Bannen")
                    ...
                    .onSubmit((e, r) -> {
                        ...
                        this.getPlugin().getTinyRabbit().sendAndReceive((data) -> {
                            switch (GuardianCalls.valueOf(data.getKey().toUpperCase())) {
                                case CALLBACK_BAN_IS_BANNED:
                                    sender.sendMessage(Language.get("guardian.ban.is.banned", target));
                                    break;
                                case CALLBACK_BAN_EXECUTED:
                                    sender.sendMessage(Language.get("guardian.ban.executed", target, reason, data.getData()[1]));
                                    break;
                            }
