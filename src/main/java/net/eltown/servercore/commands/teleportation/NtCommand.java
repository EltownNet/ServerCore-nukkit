...
public class NtCommand extends PluginCommand<ServerCore> {

    public NtCommand(final ServerCore owner) {
        super("nt", owner);
        this.setDescription("Teleportiere dich schnell in den Nether");
        this.setAliases(Arrays.asList("nether", "hölle").toArray(new String[]{}));
    }
            ...
            this.getPlugin().getTinyRabbit().sendAndReceive((delivery -> {
                switch (TeleportationCalls.valueOf(delivery.getKey().toUpperCase())) {
                    ...
                    case CALLBACK_ALL_WARPS:
                        ...
                        final Warp warp = warps.get("nether");
                        if (warp.getServer().equals(this.getPlugin().getServerName())) {
                            ...
                            this.getPlugin().getTinyRabbit().send(Queue.TELEPORTATION_RECEIVE, TeleportationCalls.REQUEST_TELEPORT.name(), "WARP_NULL==&" + "nether", player.getName(), warp.getServer(), warp.getWorld(),
