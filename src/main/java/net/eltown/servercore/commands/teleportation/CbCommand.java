...
public class CbCommand extends PluginCommand<ServerCore> {

    public CbCommand(final ServerCore owner) {
        super("cb", owner);
        this.setDescription("Teleportiere dich schnell zum CityBuild");
        this.setAliases(Arrays.asList("citybuild", "plotwelt").toArray(new String[]{}));
    }
            ...
            this.getPlugin().getTinyRabbit().sendAndReceive((delivery -> {
                switch (TeleportationCalls.valueOf(delivery.getKey().toUpperCase())) {
                    ...
                    case CALLBACK_ALL_WARPS:
                        ...
                        final Warp warp = warps.get("CityBuild");
                        if (warp.getServer().equals(this.getPlugin().getServerName())) {
                            ...
                            this.getPlugin().getTinyRabbit().send(Queue.TELEPORTATION_RECEIVE, TeleportationCalls.REQUEST_TELEPORT.name(), "WARP_NULL==&" + "CityBuild", player.getName(), warp.getServer(), warp.getWorld(),
